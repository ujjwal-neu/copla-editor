...
import PlotBandPlugin from './plugins/plotbands';
...
class Dygraph {
  ...
  __init__(div, file, attrs) {
    ...
    // Activate plugins.
    this.plugins_ = [];
    const plugins = Dygraph.PLUGINS.concat(this.getOption('plugins'));
    for (var i = 0; i < plugins.length; i++) {
      ...
      const handlers = pluginInstance.activate(this);
    ...
    for (i = 0; i < this.plugins_.length; i++) {
      const plugin_dict = this.plugins_[i];
      for (eventName in plugin_dict.events) {
        if (!plugin_dict.events.hasOwnProperty(eventName)) continue;
        const callback = plugin_dict.events[eventName];

        const pair = [plugin_dict.plugin, callback];
  ...
  /**
    ...
    * @private
    */
  cascadeEvents_(name, extra_props) {
    ...
    var e = {
      ...
      preventDefault() {
        if (!e.cancelable) throw new Error('Cannot call preventDefault on non-cancelable event.');
    ...
    const callback_plugin_pairs = this.eventListeners_[name];
    if (callback_plugin_pairs) {
      for (let i = callback_plugin_pairs.length - 1; i >= 0; i--) {
        const plugin = callback_plugin_pairs[i][0];
        const callback = callback_plugin_pairs[i][1];
  ...
  /**
    ...
    * @private
    */
  getNumericOption(name, opt_seriesName) {
  ...
  /**
    ...
    * @private
    */
  getStringOption(name, opt_seriesName) {
  ...
  /**
    ...
    * @private
    */
  getBooleanOption(name, opt_seriesName) {
  ...
  /**
    ...
    * @private
    */
  getFunctionOption(name, opt_seriesName) {
    return /** @type{function(...)} */(this.getOption(name, opt_seriesName));
  }

  getOptionForAxis(name, axis) {
  ...
  /**
    ...
    */
  optionsViewForAxis_(axis) {
    const self = this;
    return function (opt) {
      let axis_opts = self.user_attrs_.axes;
  ...
  /**
    ...
    */
  xAxisExtremes() {
    const pad = this.getNumericOption('xRangePad') / this.plotter_.area.w;
    if (this.numRows() === 0) {
      return [0 - pad, 1 + pad];
    }
    let left = this.rawData_[0][0];
    let right = this.rawData_[this.rawData_.length - 1][0];
    if (pad) {
  ...
  /**
    ...
    */
  toDomYCoord(y, axis) {
    const pct = this.toPercentYCoord(y, axis);
  ...
  toDataYCoord(y, axis) {
    ...
    const area = this.plotter_.area;
    const yRange = this.yAxisRange(axis);
  ...
  toPercentYCoord(y, axis) {
    ...
    let pct;
    const logscale = this.attributes_.getForAxis('logscale', axis);
    if (logscale) {
      const logr0 = utils.log10(yRange[0]);
      const logr1 = utils.log10(yRange[1]);
  ...
  toPercentXCoord(x) {
    ...
    const xRange = this.xAxisRange();
    let pct;
    const logscale = this.attributes_.getForAxis('logscale', 'x');
  ...
  createInterface_() {
    ...
    this.mouseOutHandler_ = function (e) {
      ...
      const target = e.target || e.fromElement;
  ...
  resizeElements_() {
    ...
    const pixelRatioOption = this.getNumericOption('pixelRatio');
    const canvasScale = pixelRatioOption || utils.getContextPixelRatio(this.canvas_ctx_);
    ...
    const hiddenScale = pixelRatioOption || utils.getContextPixelRatio(this.hidden_ctx_);
  ...
  destroy() {
    ...
    for (let i = this.plugins_.length - 1; i >= 0; i--) {
      const p = this.plugins_.pop();
  ...
  setColors_() {
    ...
    const sat = this.getNumericOption('colorSaturation') || 1.0;
    const val = this.getNumericOption('colorValue') || 0.5;
    ...
    for (let i = 0; i < num; i++) {
      ...
      let colorStr = this.attributes_.getForSeries('color', label);
      if (!colorStr) {
        if (colors) {
  ...
  createRollInterface_() {
    ...
    const display = this.getBooleanOption('showRoller') ? 'block' : 'none';
  ...
  createDragInterface_() {
    ...
    const context = {
      ...
      initializeMouseDown(event, g, contextB) {
        ...
        const canvasPos = utils.findPos(g.canvas_);
    ...
    const interactionModel = this.getOption('interactionModel');

    // Self is the graph.
    var self = this;

    // Function that binds the graph and context to the handler.
    const bindHandler = function (handler) {
      return function (event) {
        handler(event, self, context);
      };
    };

    this.plugins_.forEach(p => Object.assign(interactionModel, p.plugin.interactions));

    for (const eventName in interactionModel) {
  ...
  doZoomX_(lowX, highX) {
    ...
    const minDate = this.toDataXCoord(lowX);
    const maxDate = this.toDataXCoord(highX);
  ...
  /**
    ...
    * @private
    */
  doZoomXDates_(minDate, maxDate) {
    // TODO(danvk): when xAxisRange is null (i.e. "fit to data", the animation
    // can produce strange effects. Rather than the x-axis transitioning slowly
    // between values, it can jerk around.)
    const old_window = this.xAxisRange();
    const new_window = [minDate, maxDate];
  ...
  doZoomY_(lowY, highY) {
    ...
    const oldValueRanges = this.yAxisRanges();
    const newValueRanges = [];
    for (let i = 0; i < this.axes_.length; i++) {
      const hi = this.toDataYCoord(lowY, i);
      const low = this.toDataYCoord(highY, i);
  ...
  /**
    ...
    * @private
    */
  static zoomAnimationFunction(frame, numFrames) {
    const k = 1.5;
  ...
  resetZoom() {
    ...
    let oldWindow = null;
    let newWindow = null;
    let oldValueRanges = null;
    let newValueRanges = null;
    if (dirtyX) {
  ...
  doAnimatedZoom(oldXRange, newXRange, oldYRanges, newYRanges, callback) {
    const steps = this.getBooleanOption('animatedZooms') ?
    ...
    if (oldYRanges !== null && newYRanges !== null) {
      for (step = 1; step <= steps; step++) {
        frac = Dygraph.zoomAnimationFunction(step, steps);
        const thisRange = [];
        for (let j = 0; j < this.axes_.length; j++) {
  ...
  eventToDomCoords(event) {
    ...
    const eventElementPos = utils.findPos(this.mouseEventElement_);
  ...
  findClosestRow(domX) {
    ...
    let closestRow = -1;
    const sets = this.layout_.points;
  ...
  /**
    ...
    * @private
    */
  findStackedPoint(domX, domY) {
    const row = this.findClosestRow(domX);
    let closestPoint;
    let closestSeries;
    for (let setIdx = 0; setIdx < this.layout_.points.length; ++setIdx) {
      const boundary = this.getLeftBoundary_(setIdx);
  ...
  mouseMove_(event) {
    ...
    const points = this.layout_.points;
    if (points === undefined || points === null) return;

    const canvasCoords = this.eventToDomCoords(event);
    const canvasx = canvasCoords[0];
    const canvasy = canvasCoords[1];

    const highlightSeriesOpts = this.getOption('highlightSeriesOpts');
    ...
    if (highlightSeriesOpts && !this.isSeriesLocked()) {
      ...
      const idx = this.findClosestRow(canvasx);
      selectionChanged = this.setSelection(idx);
    }

    const callback = this.getFunctionOption('highlightCallback');
  ...
  /**
    ...
    * @private
    */
  getLeftBoundary_(setIdx) {
    if (this.boundaryIds_[setIdx]) {
      return this.boundaryIds_[setIdx][0];
    }
    for (let i = 0; i < this.boundaryIds_.length; i++) {
  ...
  animateSelection_(direction) {
    ...
    const thisId = ++this.animateId;
    const that = this;
    const cleanupIfClearing = function () {
  ...
  /**
    ...
    * @private
    */
  updateSelection_(opt_animFraction) {
    ...
    if (this.getOption('highlightSeriesOpts')) {
      ...
      let alpha = 1.0 - this.getNumericOption('highlightSeriesBackgroundAlpha');
      const backgroundColor = utils.toRGB_(this.getOption('highlightSeriesBackgroundColor'));

      if (alpha) {
        // Activating background fade includes an animation effect for a gradual
        // fade. TODO(klausw): make this independently configurable if it causes
        // issues? Use a shared preference to control animations?
        const animateBackgroundFade = true;
      ...
      for (i = 1; i < labels.length; i++) {
        const r = this.getNumericOption('highlightCircleSize', labels[i]);
        if (r > maxCircleSize) maxCircleSize = r;
      }
      const px = this.previousVerticalX_;
    ...
    if (this.selPoints_.length > 0) {
      ...
      const canvasx = this.selPoints_[0].canvasx;
      ctx.save();
      for (i = 0; i < this.selPoints_.length; i++) {
        const pt = this.selPoints_[i];
        if (isNaN(pt.canvasy)) continue;

        const circleSize = this.getNumericOption('highlightCircleSize', pt.name);
        let callback = this.getFunctionOption('drawHighlightPointCallback', pt.name);
        const color = this.plotter_.colors[pt.name];
        if (!callback) {
  ...
  /**
    ...
    * to unlock it.
    */
  setSelection(row, opt_seriesName, opt_locked) {
    ...
    if (row !== false && row >= 0) {
      ...
      for (let setIdx = 0; setIdx < this.layout_.points.length; ++setIdx) {
        ...
        // in the proper place.
        const setRow = row - this.getLeftBoundary_(setIdx);
  ...
  addXTicks_() {
    ...
    const xAxisOptionsView = this.optionsViewForAxis_('x');
    const xTicks = xAxisOptionsView('ticker')(
      range[0],
      range[1],
  ...
  predraw_() {
    ...
    for (let i = 1; i < this.numColumns(); i++) {
      ...
      let series = this.dataHandler_.extractSeries(this.rawData_, i, this.attributes_);
  ...
  /**
    ...
    * @private
    */
  static stackPoints_(points, cumulativeYval, seriesExtremes, fillMethod) {
    let lastXval = null;
    let prevPoint = null;
    let nextPoint = null;
    let nextPointIdx = -1;
    ...
    const updateNextPoint = function (idx) {
  ...
  /**
    ...
    * @private
    */
  gatherDatasets_(rolledSeries, dateWindow) {
    const boundaryIds = [];
    const points = [];
    const cumulativeYval = []; // For stacked series.
    const extremes = {}; // series name -> [low, high]
    let seriesIdx;
    let sampleIdx;
    let firstIdx;
    let lastIdx;
    ...
    function binarySearch(array = [], value) {
      ...
      while (length > 1) {
        length = right - left;
        middle = Math.floor(right - length / 2);
        middleValue = array[middle][0];
        if (middleValue < value) left = middle;
        else if (value < middleValue) right = middle;
        else return middle;
      }
      return left;
    }
    ...
    for (seriesIdx = num_series; seriesIdx >= 1; seriesIdx--) {
      if (!visibility[seriesIdx - 1]) continue;
      ...
      if (dateWindow) {
        ...
        firstIdx = binarySearch(series, dateWindow[0]);
        lastIdx = binarySearch(series, dateWindow[1]);
      ...
      const seriesName = labels[seriesIdx];
      // const seriesExtremes = this.dataHandler_.getExtremeYValues(series, dateWindow, this.getBooleanOption('stepPlot', seriesName));
      const seriesExtremes = [this.axes_[0].minyval, this.axes_[0].maxyval];

      const seriesPoints = this.dataHandler_.seriesToPoints(series, seriesName, boundaryIds[seriesIdx - 1][0]);
  ...
  /**
    ...
    */
  drawGraph_() {
    const timingName = this.getStringOption('timingName');
    if (timingName) console.time(timingName);

    // This is used to set the second parameter to drawCallback, below.
    const is_initial_draw = this.is_initial_draw_;
    ...
    const packed = this.gatherDatasets_(this.rolledSeries_, this.dateWindow_);
    ...
    if (timingName) console.timeEnd(timingName);
  }
  ...
  /**
    ...
    * @private
    */
  renderGraph_(is_initial_draw) {
    if (!this.elementsCache) this.cascadeEvents_('clearChart');
  ...
  computeYAxes_() {
    ...
    for (axis = 0; axis < this.axes_.length; axis++) {
      if (axis === 0) {
        ...
        const axes = this.user_attrs_.axes;
        if (axes && axes.y2) {
  ...
  /**
    ...
    */
  computeYAxisRanges_(extremes) {
    const isNullUndefinedOrNaN = function (num) {
      return isNaN(parseFloat(num));
    };
    const numAxes = this.attributes_.numAxes();
    let ypadCompat;
    let span;
    let series;
    let ypad;
    ...
    for (var i = 0; i < numAxes; i++) {
      ...
      const logscale = this.attributes_.getForAxis('logscale', i);
      const includeZero = this.attributes_.getForAxis('includeZero', i);
      const independentTicks = this.attributes_.getForAxis('independentTicks', i);
    ...
    for (var i = 0; i < numAxes; i++) {
      ...
      if (!axis.independentTicks) {
        ...
        const p_scale = p_axis.computedValueRange[1] - p_axis.computedValueRange[0];
        const scale = axis.computedValueRange[1] - axis.computedValueRange[0];
        const tick_values = [];
        for (let k = 0; k < p_ticks.length; k++) {
          const y_frac = (p_ticks[k].v - p_axis.computedValueRange[0]) / p_scale;
          const y_val = axis.computedValueRange[0] + y_frac * scale;
  ...
  parseCSV_(data) {
    ...
    const expectedCols = this.attr_('labels').length;
      ...
      if (i === 0 && this.attr_('labels')) {
        ...
        if (all_null) {
          console.warn(`${`${"The dygraphs 'labels' option is set, but the first row " +
            "of CSV data ('"}${line}') appears to also contain `}labels. Will drop the CSV labels and use the option labels.`);
          continue;
        }
      }
      ret.push(fields);
  ...
  /**
    ...
    * @private
    */
  parseDataTable_(data) {
    const shortTextForAnnotationNum = function (num) {
    ...
    const colIdx = [];
    const annotationCols = {}; // data index -> [annotation cols]
      ...
      if (!this.getBooleanOption('errorBars')) {
        for (j = 0; j < colIdx.length; j++) {
          ...
          if (hasAnnotations &&
            ...
            ann.text = '';
            for (let k = 0; k < annotationCols[col].length; k++) {
              if (k) ann.text += '\n';
  ...
  updateOptions(input_attrs, block_redraw) {
    ...
    const attrs = Dygraph.copyUserAttrs_(input_attrs);
    ...
    const requiresNewPoints = utils.isPixelChangingOptionList(this.attr_('labels'), attrs);
  ...
  setVisibility(num, value) {
    ...
    if (numIsObject) {
      ...
      for (var i = 0; i < num.length; i++) {
        ...
        else if (num[i] < 0 || num[i] >= x.length) {
  ...
  removeTrackedEvents_() {
    if (this.registeredEvents_) {
      for (let idx = 0; idx < this.registeredEvents_.length; idx++) {
        const reg = this.registeredEvents_[idx];
